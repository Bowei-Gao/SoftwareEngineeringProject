...
class Account(models.Model):
    ...
    avatar = models.CharField(max_length=500, blank=True, null=True)  # 头像 - base64储存方式
    background = models.CharField(max_length=1000, blank=True, null=True)  # 背景图片 - bas64储存方式
    signature = models.CharField(max_length=50, blank=True, null=True)  # 签名或个人简介
    NORMAL = 'normal'
    SUPER = 'super'
    ACCOUNT_TYPE = [
        (NORMAL, '普通用户'),
...
class FocusAccount(models.Model):  # 用户关注别人，多对多关系，使用一张表专门记录
    focusing = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id', related_name='focusing_id')
    focused = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id', related_name='focused_id')
...
class Category2(models.Model):  # 第二层分类
    ...
    category1 = models.ForeignKey('Category1', on_delete=models.CASCADE, to_field='category1_id')
...
class Topic(models.Model):  # 主题
    ...
    # topic_id = models.OneToOneField('Topic', on_delete=models.CASCADE, to_field='topic_id', primary_key=True)
    category2 = models.ForeignKey('Category2', on_delete=models.CASCADE, to_field='category2_id')
    account = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id')  # 发送者
    content = models.TextField()  # 内容
    create_time = models.DateTimeField(auto_now=timezone.now, null=True)  # 创建时间，用以排序。一般从最新的信息到较老的信息排序
    tags = models.BigIntegerField(null=True, blank=True)  # 话题标签，请使用一个整数表示该话题的所有标签，最长64位。这个暂时不知道是干嘛用的
    legal = models.BooleanField(default=True)  # 合法性，当前内容被举报不合法时，不应该对非主题/回复/评论发布者显示内容
    up_vote_count = models.IntegerField(default=0, null=True, blank=True)  # 主题被点赞计数
...
class FocusTopic(models.Model):  # 用户关注别人，多对多关系，使用一张表专门记录
    topic = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id')
    focused = models.ForeignKey('Topic', on_delete=models.CASCADE, to_field='topic_id')
...
class TopicAttitude(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['topic_id', 'account_id'], name='unique_topic_attitude')
            # 主题id和账号的联合唯一约束, 一个账号对同一个主题只能有一种态度
        ]
...
class Reply(models.Model):
    ...
    # reply_id = models.OneToOneField('Topic', on_delete=models.CASCADE, to_field='topic_id', primary_key=True)   # Reply id
    topic = models.ForeignKey('Topic', on_delete=models.CASCADE, to_field='topic_id',
                              related_name='master_id')  # 父消息ID
    account = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id')  # 评论者
    parent_reply = models.ForeignKey("self", on_delete=models.CASCADE, to_field='reply_id', blank=True, null=True)
    content = models.TextField()
    create_time = models.DateTimeField(auto_now=timezone.now, null=True,blank=True)
...
class ReplyAttitude(models.Model):
    reply = models.ForeignKey('Reply', on_delete=models.CASCADE, to_field='reply_id')
    account = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id')
    attitude = models.BooleanField()

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['reply_id', 'account_id'], name='unique_reply_attitude')
            # 消息id和账号的联合唯一约束, 一个账号对同一个消息只能有一种态度
        ]
...
class Remind(models.Model):  # 提醒管理统一格式为：被提醒者、发生时间、提醒内容三部分，具体细分种类由提醒内容决定
    ...
    receiver = models.ForeignKey('Account', on_delete=models.CASCADE, to_field='account_id')  # 被提醒者
...
class Report(models.Model):
    ...
    message = models.ForeignKey('Topic', on_delete=models.SET_NULL, null=True, to_field='topic_id')
