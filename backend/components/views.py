...
from components.publicPaing import publicPagination
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if user.password == password:
                ...
                Token.objects.update_or_create(name=nickname, defaults={
                    'token': token,
                    'time': timezone.now()
                })
                data = {
            ...
            return Response({"code": 1000, "msg": "账号不存在"})
...
# class updateUserInfo(UpdateModelMixin,RetrieveModelMixin,  GenericViewSet):  # DONE
#     """
...
class userInfo(ModelViewSet):  # DONE
    """
    向 infou/account_id/ 发送 PUT/PATCH 以进行更新
    """
    queryset = Account.objects.all()
...
class userTopic(ModelViewSet):  # DONE
    serializer_class = TopicSerializer
    queryset = Topic.objects.all()
    pagination_class = publicPagination
    filter_backends = (DjangoFilterBackend,)
    filter_fields = ('topic_id', 'account')

    # def get(self, request):
    #     queryset = Topic.objects.all()
    #     page_obj = pagination.PageNumberPagination()
    #     page_data = page_obj.paginate_queryset(queryset, request)
    #     ser_obj = TopicSerializer(page_data, many=True)
    #     return page_obj.get_paginated_response(ser_obj.data)


class useReply(ModelViewSet):
    queryset = Reply.objects.filter(parent_reply=None)
    serializer_class = ReplySerializer
    filter_backends = (DjangoFilterBackend,)
    filter_fields = ('reply_id', 'topic',)
...
class useRemind(ModelViewSet):  # DONE
...
class getMsgsFromUser(ModelViewSet):  # DONE
    queryset = Topic.objects.all()
    serializer_class = TopicSerializer
...
class useFocusFromUser(ModelViewSet):
...
class collectReply(ModelViewSet):  # DONE
...
class getRemindFromUser(ModelViewSet):  # DONE
    queryset = Remind.objects.all()
    serializer_class = RemindSerializer
...
class getCategory(ModelViewSet):  # DONE
    ...
    def get(self, request, *args, **kwargs):
        ...
        for cat in cat2:
            categories.append([cat.category2_name, Category1.objects.filter(
                category1_id=cat.category1_id.category1_id).first().category1_name, cat.category2_id])
        return Response(data=categories, status=status.HTTP_200_OK)


class createCategory1(ModelViewSet):  # backdoor
    queryset = Category1.objects.all()
    serializer_class = CategorySerializer


class createCategory2(ModelViewSet):  # backdoor
    queryset = Category2.objects.all()
    serializer_class = Category2Serializer
    filter_backends = (DjangoFilterBackend,)
    filter_fields = ('category1',)


# 分类主题
class getTopicFromCategory(ModelViewSet):  # DONE
    queryset = Topic.objects.all()
    serializer_class = TopicSerializer
...
class createFocusFromTopic(ModelViewSet):  # DONE
    queryset = FocusTopic.objects.all()
    serializer_class = FocusTopicSerializer
...
class createReportFromMsg(ModelViewSet):  # DONE
...
class createAttitudeFromMsg(ModelViewSet):  # DONE
...
class getReplyFromMsg(ModelViewSet):  # DONE
    queryset = Reply.objects.all()
    serializer_class = ReplySerializer
...
class dealReport(ModelViewSet):  # DONE
    queryset = Remind.objects.all()
    serializer_class = RemindSerializer
